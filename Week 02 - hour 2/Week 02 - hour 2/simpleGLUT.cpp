...
//			glVertex2f(0.50f, 1.50f);
			...
			// same triangle as the green one but translated x by -1, y by +1
			// (and zero by +0)
			glVertex2f(-0.50f, 1.50f);
			glVertex2f(0.0f, 1.50f);
			glVertex2f(-0.50f, 2.50f);
		glEnd();
#else

		//	save the current reference frame (origin and axes)
		glPushMatrix();
		...
		//  angle (degree), axis of rotation (x, y, z)
		glRotatef(30, 0.f, 0.f, 1.f);
		myTriangle->draw();
...
void myInit(void)
{
	...
	glutAddMenuEntry("black background", FIRST_SUBMENU_ITEM);
	glutAddMenuEntry("dark blue background", SECOND_SUBMENU_ITEM);
...
//    glutIdleFunc(myIdle);
	...
	//       delay ms    callback func   value (int)
